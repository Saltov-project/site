...
if (have_posts()) 
{
	...
	if (is_category()){
		...
		$title .= __('Архив автора', THEME_NS);
	...
	$title = '';
	if ( is_category() ) {
		$title .= sprintf(__("Извините, но в рубрике % еще нет записей.", THEME_NS), single_cat_title('',false));
	} else if ( is_date() ) { 
		$title .= __("Извините, но записей с этой датой не существует.", THEME_NS);
	} else if ( is_author() ) { 
		$userdata = get_userdatabylogin(get_query_var('author_name'));
		$title .= sprintf(__("Извините,нет никаких записей в рубрике %.", THEME_NS), $userdata->display_name);
	} else {
